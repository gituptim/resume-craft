...
export default function ModernTemplate({ data }: { data: ResumeData }) {
  return (
    ...
    >
      {/* Left Sidebar */}
      <div className="w-[32%] min-h-full p-[16mm] pt-[20mm]" style={{ backgroundColor: '#F9F6F0' }}>
        {/* Name block */}
        <div className="mb-8">
          <h1
            style={{ fontSize: `${data.layout.headingFontSize}pt` }}
            className="font-bold text-ink-primary leading-tight mb-2"
          >
            {data.personal.name || '您的姓名'}
          </h1>
          <p className="text-[10pt] text-ink-secondary">{data.personal.title || '目标职位'}</p>
        </div>

        {/* Contact */}
        <div className="mb-8 space-y-2.5">
          {data.personal.email && (
            <div className="flex items-center gap-2 text-[9pt] text-ink-secondary">
              <Mail className="w-3.5 h-3.5 shrink-0 text-ink-tertiary" />
              <span className="break-all">{data.personal.email}</span>
            </div>
          )}
          {data.personal.phone && (
            <div className="flex items-center gap-2 text-[9pt] text-ink-secondary">
              <Phone className="w-3.5 h-3.5 shrink-0 text-ink-tertiary" />
              <span>{data.personal.phone}</span>
            </div>
          )}
          {data.personal.location && (
            <div className="flex items-center gap-2 text-[9pt] text-ink-secondary">
              <MapPin className="w-3.5 h-3.5 shrink-0 text-ink-tertiary" />
              <span>{data.personal.location}</span>
            </div>
          )}
          {data.personal.website && (
            <div className="flex items-center gap-2 text-[9pt]">
              <Globe className="w-3.5 h-3.5 shrink-0 text-ink-tertiary" />
              <span className="text-themeaccent break-all">{data.personal.website}</span>
            </div>
          )}
          {data.personal.customFields.map((field) => (
            <div key={field.id} className="flex items-center gap-2 text-[9pt] text-ink-secondary">
      ...
      {/* Right Content */}
      <div className="flex-1 p-[16mm] pt-[20mm]">
